...
import { clearAllData, commandOriginToPlayer, getPlayerId, isFeatureEnabled, isOperator, saveShops, setPlayerStatById, state, tell, } from "./storage";
...
export function registerCustomCommands(startupEvent) {
    ...
    registry.registerCommand({
        ...
        if (actionKey === "list") {
            ...
            if (ids.length === 0)
                return { status: 0, message: "No crates configured." };
            for (const id of ids)
                tell(player, `§7- §e${id}`);
    ...
    registry.registerCommand({
        ...
        if (!isFeatureEnabled("items"))
            return { status: 1, message: "TauItems are disabled." };
        if (!isOperator(player))
            return { status: 1, message: "Operator required." };
        const actionKey = String(action ?? "").trim().toLowerCase();
        if (actionKey === "list") {
            ...
            if (ids.length === 0)
                return { status: 0, message: "No TauItems configured." };
            for (const id of ids)
                tell(player, `§7- §e${id}`);
    ...
    registry.registerCommand({
        ...
        if (!isFeatureEnabled("generators"))
            return { status: 1, message: "Generators are disabled." };
        system.run(async () => {
    ...
    registry.registerCommand({
        ...
        if (!player)
            return { status: 1, message: "This command can only be used by a player." };
        if (!isFeatureEnabled("warps"))
            return { status: 1, message: "Warps are disabled." };
        system.run(async () => {
    ...
    registry.registerCommand({
        ...
        if (!warp)
            return { status: 1, message: `Warp "${name}" not found.` };
        system.run(async () => {
    ...
    registry.registerCommand({
        ...
        if (!isFeatureEnabled("plots"))
            return { status: 1, message: "Plots are disabled." };
        if (!isFeatureEnabled("plotTp"))
            return { status: 1, message: "Plot teleport is disabled." };
        system.run(async () => {
    ...
    registry.registerCommand({
        ...
        if (!isOperator(player))
            return { status: 1, message: "Operator required." };
        if (!isFeatureEnabled("prune"))
            return { status: 1, message: "Prune is disabled." };
    ...
    registry.registerCommand({
        ...
        if (!isFeatureEnabled("teams"))
            return { status: 1, message: "Teams are disabled." };
        const act = String(action ?? "").trim().toLowerCase();
        if (!act || act === "ui" || act === "menu" || act === "open") {
            system.run(async () => {
        ...
        if (act === "invite") {
            ...
            if (!team)
                return { status: 1, message: "You are not in a team." };
            const targetName = String(arg1 ?? "").trim();
            if (!targetName)
                return { status: 1, message: "Usage: /tau:team invite <player>" };
            const target = world.getAllPlayers().find((p) => p.name.toLowerCase() === targetName.toLowerCase());
            if (!target)
                return { status: 1, message: `Player "${targetName}" not online.` };
        ...
        if (act === "kick") {
            ...
            const targetName = String(arg1 ?? "").trim();
            if (!targetName)
                return { status: 1, message: "Usage: /tau:team kick <player>" };
            const target = world.getAllPlayers().find((p) => p.name.toLowerCase() === targetName.toLowerCase());
            if (!target)
                return { status: 1, message: `Player "${targetName}" not online.` };
    ...
    registry.registerCommand({
        ...
        if (!isFeatureEnabled("tpa"))
            return { status: 1, message: "TPA is disabled." };
        ...
        if (!online)
            return { status: 1, message: `Player "${targetName}" is not online.` };
    ...
    registry.registerCommand({
        ...
        if (!isFeatureEnabled("homes"))
            return { status: 1, message: "Homes are disabled." };
    ...
    registry.registerCommand({
        ...
        if (homes.length === 0)
            return { status: 0, message: "No homes set." };
    ...
    registry.registerCommand({
        ...
        if (!player)
            return { status: 1, message: "This command can only be used by a player." };
        if (!isFeatureEnabled("pay"))
            return { status: 1, message: "Pay is disabled." };
        const targetName = String(target ?? "").trim();
        if (!targetName) {
            system.run(async () => {
        ...
        if (!Number.isFinite(parsed))
            return { status: 1, message: "Usage: /tau:pay <player> <amount>" };
    ...
    registry.registerCommand({
        ...
        if (!player)
            return { status: 1, message: "This command can only be used by a player." };
        if (!isFeatureEnabled("playerConfig"))
            return { status: 1, message: "Player config is disabled." };
        system.run(async () => {
    ...
    registry.registerCommand({
    ...
    }, (origin, target, stat, value) => {
        const player = commandOriginToPlayer(origin);
        const targetName = String(target ?? "").trim();
        if (!isFeatureEnabled("stats"))
            return { status: 1, message: "Stats are disabled." };
        if (!targetName)
            return { status: 1, message: "Usage: /tau:stats <player> [stat] [value]" };
        ...
        if (!Number.isFinite(parsed))
            return { status: 1, message: "Value must be a number." };
        setPlayerStatById(getPlayerId(matched), stat, parsed);
    ...
    registry.registerCommand({
        ...
        if (!player && !targetName)
            return { status: 1, message: "Usage: /tau:profile <player>" };
