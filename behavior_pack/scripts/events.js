...
import { asPlayer, incrementStat, formatChatMessage, getInventoryContainer, getPlayerStats, getPlayerStatsById, getPlayerId, getMenuIdFromTags, getMenuIdFromNameTag, isFeatureEnabled, normalizeKey, state, saveModeration, tell, } from "./storage";
const generatorMenuOpenByPlayerId = new Map();
const pendingJoinInitializationByName = new Set();
function openGeneratorMenuOnce(player, placed) {
    if (!player)
        return;
    const playerId = getPlayerId(player);
    const now = Date.now();
    const lastOpenAt = generatorMenuOpenByPlayerId.get(playerId) ?? 0;
    if (now - lastOpenAt < 600)
        return;
...
function getPlayerEnderContainer(player) {
    try {
...
function cacheModerationInspectionSnapshot(player) {
    var _a;
    if (!player)
        return;
...
function initializePlayerJoinState(player) {
    if (!player)
        return;
    ...
    if (!ensured.ok)
        showPlotError(player, ensured.message);
    ...
    if (ownerId && spawnedInSlot) {
        ...
        if (assignedSlotId && assignedSlotId !== spawnedInSlot.id) {
            ...
            if (!moved.ok)
                showPlotError(player, moved.message);
        }
    }
}
export function initializeOnlinePlayersAfterReload() {
    for (const player of getCachedPlayers()) {
        initializePlayerJoinState(player);
    }
}
function schedulePlayerJoinInitialization(playerName) {
    schedulePlayerJoinInitializationRetry(playerName, 0);
}
function schedulePlayerJoinInitializationRetry(playerName, attempt) {
    if (attempt === 0) {
        if (pendingJoinInitializationByName.has(playerName))
            return;
...
const lastSampleByPlayerId = {};
const lastSeenPlotByPlayerId = {};
...
function resolveItemMenu(itemStack) {
    ...
    if (tagged)
        return tagged;
    return (state.binds.itemBinds[itemStack.typeId] ??
        state.binds.itemBinds[normalizeKey(itemStack.typeId)]);
}
function resolveEntityMenu(entity) {
...
export function registerEventInterceptors() {
    ...
    world.afterEvents.playerLeave.subscribe((event) => {
        ...
        if (playerId)
            releasePlayerPlotById(playerId);
    ...
    world.afterEvents.itemUse.subscribe((event) => {
        ...
        if (player && isFeatureEnabled("items")) {
            ...
            if (handled.matched && handled.message)
                tell(player, handled.message);
        }
    ...
    world.beforeEvents.playerInteractWithBlock.subscribe((event) => {
        ...
        if (isFeatureEnabled("items")) {
            ...
            if (handled.matched) {
                if (handled.cancel)
                    event.cancel = true;
                if (handled.message)
                    tell(event.player, handled.message);
                if (handled.cancel)
                    return;
            }
        }
        if (isFeatureEnabled("crates")) {
            ...
            if (crate.handled) {
                event.cancel = true;
                if (crate.message)
                    tell(event.player, crate.message);
                return;
            }
        }
        if (isFeatureEnabled("generators")) {
            ...
            if (event.player.isSneaking && !hasGeneratorItem && isGeneratorBlock(event.block.location, event.player.dimension.id)) {
                ...
                if (placed)
                    openGeneratorMenuOnce(event.player, placed);
                return;
            }
        }
        if (!isFeatureEnabled("generators"))
            return;
        const heldStack = getInventoryContainer(event.player)?.getItem(event.player.selectedSlotIndex);
        if (!describeGeneratorStack(heldStack))
            return;
        event.cancel = true;
        system.run(() => {
    ...
    world.afterEvents.entityDie.subscribe((event) => {
        ...
        if (dead) {
            ...
            if (deadStats.longestKillstreak < deadStats.killstreak)
                deadStats.longestKillstreak = deadStats.killstreak;
    ...
    world.beforeEvents.entityHurt.subscribe((event) => {
        ...
        if (!victimTeam)
            return;
        if (victimTeam.ownerPlayerId !== getPlayerId(attacker) && !victimTeam.memberPlayerIds.includes(getPlayerId(attacker)))
            return;
    ...
    system.runInterval(() => {
        ...
        for (const player of getCachedPlayers()) {
            ...
            if (prev) {
                ...
                if (dist > 0)
                    incrementStat(player, "distanceTraveled", dist);
            }
            lastSampleByPlayerId[id] = { x: player.location.x, y: player.location.y, z: player.location.z };
    ...
    system.runInterval(() => {
        if (!isFeatureEnabled("plots"))
            return;
        if (!state.plots.config.autoBuild.showEnterTitle)
            return;
        ...
        for (const player of getCachedPlayers()) {
            ...
            const slot = getPlotSlotsList().find((s) => expanded.x >= s.min.x - radius && expanded.x <= s.max.x + radius &&
                expanded.y >= s.min.y - radius && expanded.y <= s.max.y + radius &&
                expanded.z >= s.min.z - radius && expanded.z <= s.max.z + radius) ?? getPlotForLocation(player.location);
