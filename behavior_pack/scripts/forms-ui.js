...
import { ACTION_TYPES, CODE_TO_COLOR_NAME, ICONS, ICON_DEV_OPTIONS, RANK_COLORS, RANK_COLOR_CODES, WORKING_ICON_OPTIONS, isWorkingIconPath, } from "./tau-models";
import { assignRank, findForm, getRankById, getPlayerId, getPlayerRank, getPlayerStats, getPlayerStatsById, getInventoryContainer, isFeatureEnabled, isOperator, normalizeKey, removeRank, saveBinds, saveChat, saveConfig, saveCombat, saveForms, savePrune, saveProfiles, savePlots, saveRanks, saveModeration, saveCrates, saveTauItems, saveGenerators, setDefaultRank, state, tell, } from "./storage";
...
import { iconForElement, optionalIcon } from "./tau-ui";
function normalizeItemId(value) {
    return String(value ?? "").trim().toLowerCase();
}
function isBannedItemId(itemId) {
    const normalized = normalizeItemId(itemId);
    return state.moderation.bannedItems.some((entry) => normalizeItemId(entry.itemId) === normalized);
}
function clearBannedHeldItem(player) {
    ...
    if (!held)
        return false;
    if (!isBannedItemId(held.typeId))
        return false;
    container.setItem(player.selectedSlotIndex, undefined);
    return true;
}
function clearBannedInventoryItems(player) {
    const container = player.getComponent(EntityComponentTypes.Inventory)?.container;
    if (!container)
        return 0;
    let removed = 0;
    for (let slot = 0; slot < container.size; slot++) {
        ...
        if (!stack || !isBannedItemId(stack.typeId))
            continue;
...
function getContainerItems(container, slotCount) {
    const items = [];
...
function formatStackLine(slot, stack) {
...
function saveModerationInspectionSnapshot(playerName, inventory, enderChest) {
    var _a;
    (_a = state.moderation).inspectionSnapshots ?? (_a.inspectionSnapshots = {});
...
function getInspectionSnapshotKey(playerName) {
    return String(playerName ?? "").trim().toLowerCase();
}
function createSnapshotContainer(items) {
    return {
        ...
        getItem(slot) {
            ...
            if (entry.nameTag)
                stack.nameTag = entry.nameTag;
            if (entry.lore)
                stack.setLore(entry.lore);
            return stack;
        },
        setItem(slot, item) {
...
function getEnderChestContainer(player) {
    try {
        const ender = player.getComponent("ender_inventory");
...
async function showContainerInspector(player, title, container, slotCount) {
    while (true) {
        ...
        if (!response || response.canceled || response.selection === undefined || response.selection === 1)
            return;
        if (items.length === 0)
            continue;
        const picker = new ActionFormData().title(`${title} - Delete Item`).body("§7Select an item to delete.");
        for (const entry of items)
            picker.button(formatStackLine(entry.slot, entry.stack), ICONS.delete);
        picker.button("Back", ICONS.back);
        const picked = await picker.show(player).catch(() => undefined);
        if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= items.length)
            continue;
...
async function showOnlinePlayerInspector(player) {
    while (true) {
        ...
        const form = new ActionFormData()
            ...
            .button("Back", ICONS.back);
        const response = await form.show(player).catch(() => undefined);
        if (!response || response.canceled || response.selection === undefined || response.selection === 2)
            return;
        if (online.length === 0) {
        ...
        for (const target of online)
            picker.button(target.name, ICONS.item);
        picker.button("Back", ICONS.back);
        const picked = await picker.show(player).catch(() => undefined);
        if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= online.length)
            continue;
...
async function showOfflinePlayerInspector(player) {
...
function captureHeldBannedItem(player) {
    const container = player.getComponent(EntityComponentTypes.Inventory)?.container;
    const held = container?.getItem(player.selectedSlotIndex);
    if (!held)
        return undefined;
...
function getHeldItemSnapshot(player) {
    const container = player.getComponent(EntityComponentTypes.Inventory)?.container;
    const held = container?.getItem(player.selectedSlotIndex);
    if (!held)
        return undefined;
    ...
    const enchantComp = held.getComponent(ItemComponentTypes.Enchantable);
    if (enchantComp?.getEnchantments) {
        try {
            snapshot.enchantments = enchantComp.getEnchantments().map((entry) => ({ id: entry.type?.id ?? entry.typeId, level: entry.level }));
        }
        catch {
            // ignore
        }
    ...
    try {
        const placeComp = held.getComponent("minecraft:can_place_on");
        if (placeComp?.blocks)
            snapshot.canPlaceOn = Array.isArray(placeComp.blocks) ? placeComp.blocks.slice() : String(placeComp.blocks).split(",");
    }
    catch {
        // ignore
    }
    try {
        const destroyComp = held.getComponent("minecraft:can_destroy");
        if (destroyComp?.blocks)
            snapshot.canDestroy = Array.isArray(destroyComp.blocks) ? destroyComp.blocks.slice() : String(destroyComp.blocks).split(",");
    }
    catch {
        // ignore
    }
...
function heldItemToCrateReward(player, label, weight, amount) {
...
function getCrateRewardChanceText(reward, totalWeight) {
    if (totalWeight <= 0)
        return "0%";
    const chance = (reward.weight / totalWeight) * 100;
    return `${chance.toFixed(chance >= 10 ? 1 : 2)}%`;
}
function applyHeldItemSnapshotToGenerator(def, snapshot) {
    if (!snapshot)
        return;
...
async function showModerationPlayerCleaner(player) {
    ...
    for (const target of online)
        picker.button(target.name, ICONS.delete);
    picker.button("Back", ICONS.back);
    const picked = await picker.show(player).catch(() => undefined);
    if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= online.length)
        return;
...
async function editFormElement(player, form, index) {
...
async function editElementModal(player, form, index) {
    const element = form.elements[index];
    if (!element)
        return;
...
export async function openFormById(player, menuId) {
    ...
    if (form.layout === "action") {
        const buttons = form.elements.filter((element) => element.kind === "button");
        const actionForm = new ActionFormData().title(form.title);
        if (form.body)
            actionForm.body(form.body);
        for (const button of buttons) {
            const icon = optionalIcon(button.iconPath) ?? undefined;
            if (icon)
                actionForm.button(button.text, icon);
            else
                actionForm.button(button.text);
        }
    ...
    for (let i = 0; i < handlers.length; i++) {
        ...
        const shouldExecute = typeof selectedValue === "boolean"
            ? selectedValue
            : selectedValue !== undefined && String(selectedValue).length > 0;
        if (!shouldExecute && handler.action !== "CLOSE")
            continue;
        await runBoundAction(player, handler.action, handler.value, selectedValue);
    }
}
async function runBoundAction(player, action, rawValue, selectedValue) {
...
export async function showCreatorMenu(player) {
...
export async function showIconDevMenu(player) {
    while (true) {
        const form = new ActionFormData()
            .title("§6Icon Dev§r")
        ...
        if (!response || response.canceled || response.selection === undefined)
            return;
        if (response.selection >= ICON_DEV_OPTIONS.length)
            return;
        ...
        if (!previewResponse || previewResponse.canceled || previewResponse.selection !== 0)
            continue;
    }
}
export async function showModerationMenu(player) {
    if (!isOperator(player)) {
    ...
    while (true) {
        ...
        if (response.selection === 1) {
            ...
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= banned.length)
                continue;
            ...
            if (!detailResponse || detailResponse.canceled || detailResponse.selection !== 0)
                continue;
        ...
        if (response.selection === 3) {
            ...
            if (!pickedMode || pickedMode.canceled || pickedMode.selection === undefined || pickedMode.selection === 2)
                continue;
...
export async function showPlotManager(player) {
    if (!isOperator(player)) {
    ...
    while (true) {
        ...
        if (response.selection === 11) {
            ...
            if (result.ok)
                tell(player, "Layout valid. No overlaps found.");
            else {
                tell(player, "Layout has issues:");
                for (const line of result.errors.slice(0, 12))
                    tell(player, `- ${line}`);
            }
            continue;
        }
        if (response.selection === 12) {
            ...
            if (lines.length === 0)
                tell(player, "No plot slots configured.");
            else {
...
export async function showPlotPlayerMenuFromCreator(player) {
    await showPlotPlayerMenu(player);
}
async function showPlotAutoBuildOptions(player) {
    while (true) {
...
async function showPlotForceRelease(player) {
    while (true) {
...
async function showPlotReassign(player) {
    ...
    for (const p of players)
        pickPlayer.button(p.name, ICONS.menu);
    pickPlayer.button("Back", ICONS.back);
    const pResp = await pickPlayer.show(player).catch(() => undefined);
    if (!pResp || pResp.canceled || pResp.selection === undefined)
        return;
    if (pResp.selection >= players.length)
        return;
...
async function showPlotTeleport(player) {
    while (true) {
        const entries = Object.entries(state.plots.slots);
...
async function showPlotSlotEditor(player) {
    while (true) {
        const entries = Object.entries(state.plots.slots);
        ...
        if (!result || result.canceled || !result.formValues)
            continue;
        const parse = (raw) => raw.trim().split(/\s+/).map((n) => Number(n));
...
export async function showRankMenu(player) {
    if (!isOperator(player)) {
...
async function showCreateBaseForm(player, layout) {
    const modal = new ModalFormData()
...
async function showActionButtonCreator(player, form) {
    const modal = new ModalFormData()
...
async function showModalElementCreator(player, form) {
    const pickType = await new ActionFormData()
        ...
        .show(player)
        .catch(() => undefined);
    if (!pickType || pickType.canceled || pickType.selection === undefined)
        return;
    if (pickType.selection === 6)
        return;
    ...
    if (!baseResponse || baseResponse.canceled || !baseResponse.formValues)
        return;
...
async function showFormEditor(player, formId) {
...
export async function showBindingsEditor(player) {
...
export async function showConfigMenu(player) {
    if (!isOperator(player)) {
    ...
    while (true) {
        ...
        const keys = ["creator", "forms", "shops", "sidebars", "bindings", "stats", "plots", "tpa", "homes", "pay", "playerConfig", "teams"];
...
export async function showPruneDataMenu(player) {
    if (!isOperator(player)) {
    ...
    while (true) {
        ...
        if (response.selection === 1) {
            ...
            if (Number.isFinite(days))
                prune.inactiveDays = days;
            savePrune();
            continue;
        }
        const flagKeys = ["stats", "profiles", "teams", "plots", "homes", "tpa", "pay", "playerSettings"];
...
export async function showTpaMenu(player) {
    ...
    tell(player, result.message);
    if (result.ok)
        tell(target, `§e${player.name} sent you a TPA request. Use /tau:tpaccept or /tau:tpdeny.`);
}
export async function showHomesMenu(player) {
    while (true) {
        ...
        for (const name of homes)
            pick.button(name, ICONS.menu);
        pick.button("Back", ICONS.back);
        const picked = await pick.show(player).catch(() => undefined);
        if (!picked || picked.canceled || picked.selection === undefined)
            continue;
        if (picked.selection >= homes.length)
            continue;
        const name = homes[picked.selection];
        if (res.selection === 1)
            tell(player, teleportHome(player, name).message);
        else
            tell(player, deleteHome(player, name).message);
    }
}
export async function showPayMenu(player) {
    ...
    for (const p of players)
        pick.button(p.name, ICONS.shop);
    pick.button("Back", ICONS.back);
    const picked = await pick.show(player).catch(() => undefined);
    if (!picked || picked.canceled || picked.selection === undefined)
        return;
    if (picked.selection >= players.length)
        return;
    ...
    if (payResult.ok)
        tell(target, `§aYou received a payment from ${player.name}.`);
}
export async function showPlayerSettingsMenu(player) {
...
export async function showSocialSettingsAdmin(player) {
    if (!isOperator(player))
        return;
...
export async function showCombatSettingsAdmin(player) {
    if (!isOperator(player))
        return;
...
async function showTeamInviteCenter(player) {
    while (true) {
        ...
        if (response.selection === 1) {
            ...
            for (const p of invitedPlayers)
                pick.button(p.name, ICONS.delete);
            pick.button("Back", ICONS.back);
            const picked = await pick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= invitedPlayers.length)
                continue;
            tell(player, revokeTeamInvite(player, invitedPlayers[picked.selection]).message);
        }
    }
}
async function showPendingTeamInvites(player) {
    while (true) {
        ...
        if (response.selection === 0) {
            ...
            for (const team of invitedTeams)
                pick.button(getTeamSummary(team), ICONS.confirm);
            pick.button("Back", ICONS.back);
            const picked = await pick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= invitedTeams.length)
                continue;
    ...
    while (true) {
        ...
        if (team) {
            ...
            if (response.selection === 2) {
                ...
                if (!subResp || subResp.canceled || subResp.selection === undefined)
                    continue;
                if (subResp.selection === 4)
                    continue;
        ...
        if (response.selection === 1) {
            ...
            for (const teamEntry of teams)
                pick.button(getTeamSummary(teamEntry), ICONS.menu);
            pick.button("Back", ICONS.back);
            const picked = await pick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= teams.length)
                continue;
...
export async function showWarpMenu(player) {
    ...
    while (true) {
        ...
        for (const warp of warps)
            pick.button(`${warp.category}: ${warp.name}`, ICONS.sidebar);
        pick.button("Back", ICONS.back);
        const picked = await pick.show(player).catch(() => undefined);
        if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= warps.length)
            continue;
        const warp = warps[picked.selection];
        tell(player, teleportToWarp(player, warp.id).message);
    }
}
export async function showGeneratorMenu(player) {
    while (true) {
        ...
        if (response.selection === 0) {
            ...
            for (const def of defs)
                pick.button(def.name, ICONS.menu);
            pick.button("Back", ICONS.back);
            const picked = await pick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= defs.length)
                continue;
...
const TAU_ITEM_TRIGGER_OPTIONS = ["use_air", "use_block", "hit_melee", "mine_block"];
const TAU_ITEM_CONSUMPTION_OPTIONS = ["none", "consume_item", "damage_durability"];
function parseTriggerCsv(value) {
...
function parseActionsJson(raw, fallback) {
    try {
...
function parseSingleActionJson(raw) {
    try {
        const parsed = JSON.parse(raw);
...
const TAU_ITEM_PARTICLE_OPTIONS = [
...
async function showTauItemActionSimpleCreate(player) {
    const form = new ActionFormData()
        .title("Simple Action")
        ...
        .button("Back", ICONS.back);
    const response = await form.show(player).catch(() => undefined);
    if (!response || response.canceled || response.selection === undefined || response.selection === 6)
        return undefined;
    if (response.selection === 0) {
        const modal = new ModalFormData()
            .title("Command Chain")
    ...
    if (response.selection === 5) {
        ...
        return { type: "aoe", radius: Number(result.formValues[0] ?? 5), mode: ["damage", "heal", "knockback"][modeIndex], amount: Number(result.formValues[2] ?? 4) };
    }
    return undefined;
}
async function showTauItemActionCustomCreate(player) {
    const modal = new ModalFormData()
...
function defaultTauItemDefinition(id, displayName, baseItemId) {
    return {
...
async function showTauItemActionsMenu(player, tauItemId) {
    while (true) {
        ...
        if (response.selection === 2 || response.selection === 3) {
            ...
            picker.button("Back", ICONS.back);
            const picked = await picker.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= def.actions.length)
                continue;
            if (response.selection === 3) {
            ...
            if (action.type === "aoe") {
                ...
                action.mode = ["damage", "heal", "knockback"][Math.max(0, Math.min(2, Math.floor(Number(result.formValues[1] ?? 0))))];
...
async function showTauItemTriggerPicker(player, currentTriggers) {
    const modal = new ModalFormData()
        .title("Set Triggers")
    ...
    if (!result || result.canceled || !result.formValues)
        return undefined;
    const selected = [];
    if (Boolean(result.formValues[0]))
        selected.push("use_air");
    if (Boolean(result.formValues[1]))
        selected.push("use_block");
    if (Boolean(result.formValues[2]))
        selected.push("hit_melee");
    if (Boolean(result.formValues[3]))
        selected.push("mine_block");
    return selected.length > 0 ? selected : ["use_air"];
}
async function showTauItemTriggerEditor(player, tauItemId) {
    while (true) {
        const def = getTauItemDefinition(tauItemId);
        if (!def) {
            tell(player, "TauItem not found.");
...
async function showTauItemCreateSimple(player) {
    const modal = new ModalFormData()
...
async function showTauItemCreateAdvanced(player) {
    const modal = new ModalFormData()
    ...
    if (!result || result.canceled || !result.formValues)
        return;
    const parsed = parseJsonText(String(result.formValues[0] ?? "{}"), defaultTauItemDefinition("fire_staff", "§cStaff of Embers", "minecraft:stick"));
...
function parseEnchantmentsText(raw) {
...
async function showTauItemEditor(player, tauItemId) {
    while (true) {
        const def = getTauItemDefinition(tauItemId);
        if (!def) {
            tell(player, "TauItem not found.");
...
export async function showTauItemsAdminMenu(player) {
    if (!isOperator(player)) {
    ...
    while (true) {
        ...
        if (response.selection === 2) {
            ...
            for (const id of ids)
                pick.button(state.tauItems.items[id]?.displayName ?? id, ICONS.edit);
            pick.button("Back", ICONS.back);
            const picked = await pick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= ids.length)
                continue;
        ...
        if (response.selection === 3) {
            ...
            for (const id of ids)
                pick.button(state.tauItems.items[id]?.displayName ?? id, ICONS.delete);
            pick.button("Back", ICONS.back);
            const picked = await pick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= ids.length)
                continue;
...
export async function showGeneratorAdminMenu(player) {
    if (!isOperator(player)) {
    ...
    while (true) {
        ...
        if (response.selection === 1) {
            ...
            for (const def of defs)
                targetPick.button(def.name, ICONS.menu);
        ...
        if (response.selection === 2) {
            ...
            for (const def of defs)
                targetPick.button(def.name, ICONS.edit);
            targetPick.button("Back", ICONS.back);
            const picked = await targetPick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= defs.length)
                continue;
        ...
        if (response.selection === 4) {
            ...
            for (const def of defs)
                targetPick.button(def.name, ICONS.settings);
            targetPick.button("Back", ICONS.back);
            const picked = await targetPick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= defs.length)
                continue;
            ...
            if (!placedPick || placedPick.canceled || placedPick.selection === undefined || placedPick.selection >= placements.length)
                continue;
        ...
        if (response.selection === 5) {
            ...
            for (const def of defs)
                targetPick.button(def.name, ICONS.delete);
            targetPick.button("Back", ICONS.back);
            const picked = await targetPick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= defs.length)
                continue;
...
async function showGeneratorTierManager(player, defId) {
    while (true) {
        ...
        if (response.selection === 1) {
            ...
            for (const tier of tiers)
                pick.button(getGeneratorTierSummary(def.id, tier.tier) ?? `Tier ${tier.tier}`, ICONS.edit);
            pick.button("Back", ICONS.back);
            const picked = await pick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= tiers.length)
                continue;
        ...
        if (response.selection === 2) {
            ...
            for (const tier of tiers)
                pick.button(getGeneratorTierSummary(def.id, tier.tier) ?? `Tier ${tier.tier}`, ICONS.delete);
            pick.button("Back", ICONS.back);
            const picked = await pick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= tiers.length)
                continue;
...
export async function showGeneratorSettingsMenu(player) {
    if (!isOperator(player)) {
...
function formatCrateLocation(entry) {
    return `${entry.dimensionId} @ ${entry.x}, ${entry.y}, ${entry.z}`;
}
async function showCrateEditor(player, crateId) {
    while (true) {
        ...
        if (response.selection === 5) {
            ...
            if (!result || result.canceled || !result.formValues)
                continue;
            crate.particlePreset = (["arcane", "ember", "frost", "void"][Math.max(0, Math.min(3, Math.floor(Number(result.formValues[0] ?? 0))))] ?? "arcane");
            saveCrates();
            continue;
        }
        if (response.selection === 6) {
...
async function showCrateRewardEditor(player, crateId) {
    while (true) {
        const crate = state.crates.crates[crateId];
        if (!crate) {
            tell(player, "Crate not found.");
        ...
        if (response.selection === 4 || response.selection === 5) {
            ...
            for (const reward of crate.rewards)
                pick.button(`${reward.label} (${reward.type})`, response.selection === 4 ? ICONS.edit : ICONS.delete);
            pick.button("Back", ICONS.back);
            const picked = await pick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= crate.rewards.length)
                continue;
            if (response.selection === 5) {
...
export async function showCrateAdminMenu(player) {
    if (!isOperator(player)) {
    ...
    while (true) {
        ...
        if (response.selection === 0) {
            ...
            state.crates.crates[id] = {
                ...
                animationPreset: (["arcane", "ember", "frost", "void"][Math.max(0, Math.min(3, Math.floor(Number(result.formValues[7] ?? 0))))] ?? "arcane"),
            ...
            if (Boolean(result.formValues[3])) {
                ...
                if (held) {
                    ...
                    if (Boolean(result.formValues[4]) && held.lore && held.lore.length > 0)
                        state.crates.crates[id].keyLoreLine = held.lore[0] ?? state.crates.crates[id].keyLoreLine;
                }
            }
            saveCrates();
            continue;
        ...
        if (response.selection === 1) {
            ...
            for (const id of validCrateIds)
                pick.button(state.crates.crates[id].displayName, ICONS.edit);
            pick.button("Back", ICONS.back);
            const picked = await pick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= validCrateIds.length)
                continue;
        ...
        if (response.selection === 2) {
            ...
            for (const id of validCrateIds)
                pick.button(state.crates.crates[id].displayName, ICONS.delete);
            pick.button("Back", ICONS.back);
            const picked = await pick.show(player).catch(() => undefined);
            if (!picked || picked.canceled || picked.selection === undefined || picked.selection >= validCrateIds.length)
                continue;
...
export async function showGeneratorUpgradeMenu(player, defId, location, dimensionId) {
    const def = getGeneratorDefinition(defId);
    if (!def) {
        tell(player, "Generator not found.");
        return;
...
export async function showWarpAdminMenu(player) {
    if (!isOperator(player)) {
    ...
    while (true) {
        ...
        const warp = warps[picked.selection];
        if (response.selection === 1)
            tell(player, setWarpLocation(player, warp.id).message);
        if (response.selection === 2)
            tell(player, deleteWarp(warp.id).message);
        if (response.selection === 3) {
            tell(player, `${warp.name} @ ${warp.dimensionId} (${warp.position.x}, ${warp.position.y}, ${warp.position.z})`);
        }
    }
}
export async function showPlotPlayerMenu(player) {
    ...
    if (response.selection === 0) {
        ...
        if (slot)
            tell(player, teleportPlayerToSlot(player, slot.id).message);
        return;
    }
    if (response.selection === 1) {
        const slot = teamPlot ?? mySlot;
        ...
        if (teamPlot && team)
            tell(player, `Team: ${team.name}`);
    }
}
export async function showRankEditor(player, rankId) {
    if (!isOperator(player)) {
        tell(player, "Only operators can adjust ranks.");
        return;
    }
    ...
    const colorIndex = RANK_COLORS.indexOf(colorName);
    ...
    rank.permissions = permText ? permText.split(",").map((p) => p.trim()).filter(Boolean) : [];
    const chatFmt = String(result.formValues[6] ?? "").trim();
    rank.chatFormat = chatFmt || undefined;
    saveRanks();
    tell(player, `Rank "${rankId}" updated.`);
}
export async function showRankManager(player) {
    if (!isOperator(player)) {
        tell(player, "Only operators can adjust ranks.");
        return;
    }
    while (true) {
        ...
        if (response.selection === rankKeys.length + 1) {
            ...
            if (!pickResponse || pickResponse.canceled || pickResponse.selection === undefined)
                continue;
        ...
        if (response.selection === rankKeys.length + 2)
            return;
        ...
        if (!editResponse || editResponse.canceled || editResponse.selection === undefined)
            continue;
        if (editResponse.selection === 0) {
            ...
            for (const [pname, rid] of Object.entries(state.ranks.playerRanks)) {
                if (rid === selectedRank.id)
                    delete state.ranks.playerRanks[pname];
            }
            saveRanks();
...
async function showCreateRankForm(player) {
    if (!isOperator(player)) {
        tell(player, "Only operators can adjust ranks.");
        return;
    }
    ...
    state.ranks.ranks[id] = {
        ...
        permissions: permText ? permText.split(",").map((p) => p.trim()).filter(Boolean) : [],
        chatFormat: chatFmt || undefined,
    };
    saveRanks();
    tell(player, `Rank "${id}" created.`);
}
export async function showPlayerRankAssign(player) {
    if (!isOperator(player)) {
        tell(player, "Only operators can adjust ranks.");
        return;
    }
    while (true) {
        ...
        if (!rankResponse || rankResponse.canceled || rankResponse.selection === undefined)
            continue;
        ...
        if (rankResponse.selection === rankKeys.length + 1)
            continue;
...
export async function showChatConfig(player) {
    while (true) {
        const form = new ModalFormData()
            ...
            .submitButton("Save");
        const result = await form.show(player).catch(() => undefined);
        if (!result || result.canceled || result.formValues === undefined)
            return;
...
export async function showProfileBrowser(player) {
    while (true) {
        const form = new ActionFormData()
        ...
        if (!response || response.canceled || response.selection === undefined)
            return;
        if (response.selection >= world.getAllPlayers().length)
            return;
...
export async function showPlayerProfileViewer(player, targetName) {
...
export async function showPlayerProfileEditor(player, targetName) {
    const target = world.getAllPlayers().find((entry) => entry.name.toLowerCase() === targetName.toLowerCase());
    if (!target) {
...
function toggleSection(profile, section) {
    const index = profile.sections.indexOf(section);
    if (index >= 0)
        profile.sections.splice(index, 1);
    else
