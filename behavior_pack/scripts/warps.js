...
import { saveWarps, state } from "./storage";
function normalizeWarpId(value) {
...
export function createWarp(player, nameRaw, categoryRaw) {
    if (!state.warps.config.enabled)
        return { ok: false, message: "Warps are disabled." };
    const name = String(nameRaw ?? "").trim();
    if (!name)
        return { ok: false, message: "Warp name is required." };
    const id = normalizeWarpId(name);
    if (!id)
        return { ok: false, message: "Invalid warp name." };
    if (state.warps.warps[id])
        return { ok: false, message: "That warp already exists." };
    if (Object.keys(state.warps.warps).length >= state.warps.config.maxWarps)
        return { ok: false, message: "Warp limit reached." };
...
export function setWarpLocation(player, warpIdOrName) {
    ...
    if (!warp)
        return { ok: false, message: "Warp not found." };
...
export function deleteWarp(warpIdOrName) {
    const id = normalizeWarpId(warpIdOrName);
    const warp = state.warps.warps[id] ?? Object.values(state.warps.warps).find((entry) => normalizeWarpId(entry.name) === id);
    if (!warp)
        return { ok: false, message: "Warp not found." };
...
export function teleportToWarp(player, warpIdOrName) {
    if (!state.warps.config.enabled)
        return { ok: false, message: "Warps are disabled." };
    const id = normalizeWarpId(warpIdOrName);
    const warp = state.warps.warps[id] ?? Object.values(state.warps.warps).find((entry) => normalizeWarpId(entry.name) === id);
...
export function getWarpCategories() {
