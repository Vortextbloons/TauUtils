...
import { STORAGE_KEYS } from "./tau-models";
...
function resolvePlotLayoutOptions(options) {
...
export function setPlotSpacing(spacing) {
    state.plots.config.spacing = Math.max(0, Math.floor(spacing));
    savePlots();
}
export function buildManualGridSlots(options) {
    const origin = state.plots.config.origin;
    if (!origin)
        return { ok: false, message: "Set plot origin first." };
...
const pendingJobCountsBySlot = {};
const completedSlots = new Set();
let buildSessionTotalSlots = 0;
const snapshotQueue = [];
let cachedPlotSlots;
let cachedPlotSlotsSource;
function getPlotSlots() {
    if (cachedPlotSlotsSource === state.plots.slots && cachedPlotSlots)
...
export function getPlotSlotsList() {
    return getPlotSlots();
}
...
function hasOnlinePlotAccess(ownerPlayerId, onlineIds) {
    if (onlineIds.has(ownerPlayerId))
        return true;
...
function repairOwnedPlotSlots(ownerPlayerId, authoritativeSlotId) {
    let changed = false;
    for (const slot of getPlotSlots()) {
...
export function reconcilePlotOwnershipData() {
    ...
    for (const [ownerPlayerId, slotId] of Object.entries(nextPlayerToSlot)) {
        if (repairOwnedPlotSlots(ownerPlayerId, slotId))
            mappingsFixed += 1;
    }
    for (const [ownerPlayerId, snapshot] of Object.entries(state.plots.snapshots)) {
        if (!snapshot.generators || snapshot.generators.length === 0)
            continue;
        for (const entry of snapshot.generators) {
            if (entry.ownerPlayerId === ownerPlayerId)
                continue;
    ...
    for (const placed of Object.values(state.generators.placed)) {
        ...
        if (!ownerPlayerId)
            continue;
        if (placed.ownerPlayerId === ownerPlayerId)
            continue;
        placed.ownerPlayerId = ownerPlayerId;
        generatorsFixed += 1;
    }
    if (mappingsFixed > 0 || snapshotsFixed > 0)
        savePlots();
    if (generatorsFixed > 0)
        saveGenerators();
    return {
        ok: true,
        mappingsFixed,
...
function reconcileTeamPlotSlots() {
    let changed = false;
        ...
        for (const memberId of team.memberPlayerIds) {
            ...
            if (mappedSlotId) {
                ...
                if (mappedSlot && mappedSlot.occupiedByPlayerId === memberId) {
                    if (saveAndClearSlot(mappedSlot, memberId))
                        changed = true;
            ...
            if (savedSlot && savedSlot.occupiedByPlayerId === memberId) {
                if (saveAndClearSlot(savedSlot, memberId))
                    changed = true;
...
function canUsePlotCommandsNow() {
    try {
...
function runFillCommand(min, max, block) {
    const dim = getDimension();
    dim.runCommand(`fill ${min.x} ${min.y} ${min.z} ${max.x} ${max.y} ${max.z} ${block}`);
}
function getFillVolume(min, max) {
    return (max.x - min.x + 1) * (max.y - min.y + 1) * (max.z - min.z + 1);
}
function splitFillJob(job) {
...
function shouldSplitFillJob(job) {
...
function enqueueFill(slotId, min, max, block) {
    const job = { slotId, min, max, block };
...
function distanceToBox(location, min, max) {
...
export function processQueuedPlotBuildJobs() {
...
function buildSlotGeometry(slot) {
    try {
        ...
        if (auto.floorBlock) {
            enqueueFill(slot.id, { x: slot.min.x, y: slot.min.y - 1, z: slot.min.z }, { x: slot.max.x, y: slot.min.y - 1, z: slot.max.z }, auto.floorBlock);
        }
        if (auto.addBorders) {
...
function applyAutoBuildRoof(slot) {
    const auto = state.plots.config.autoBuild;
    if (!auto.roofBlock)
        return;
    ...
    enqueueFill(slot.id, { x: slot.min.x, y: bottom, z: slot.min.z }, { x: slot.max.x, y: top, z: slot.max.z }, auto.roofBlock);
}
function clearAutoBuildRoof(slot) {
    const auto = state.plots.config.autoBuild;
    if (!auto.roofBlock)
        return;
    const roofHeight = Math.max(1, Math.floor(auto.roofHeight));
    const top = slot.max.y;
    const bottom = Math.max(slot.min.y, top - roofHeight + 1);
    enqueueFill(slot.id, { x: slot.min.x, y: bottom, z: slot.min.z }, { x: slot.max.x, y: top, z: slot.max.z }, "air");
}
function enqueueLayoutClear(slots) {
    if (slots.length === 0)
        return;
...
export function buildPlotGeometry(slotId) {
    ...
    if (ids.length === 0)
        return { ok: false, message: "No plot slots configured." };
...
export function updatePlotAutoBuildSettings(partial) {
    state.plots.config.autoBuild = { ...state.plots.config.autoBuild, ...partial };
    savePlots();
}
export function setSlotManualBounds(slotId, cornerA, cornerB) {
    const slot = state.plots.slots[slotId];
    if (!slot)
        return false;
...
export function validatePlotLayout() {
    const errors = [];
    const slots = getPlotSlots();
...
export function getPlotStatusLines() {
    const lines = [];
...
export function forceReleasePlot(slotId) {
    const slot = state.plots.slots[slotId];
    if (!slot)
        return false;
    if (slot.occupiedByPlayerId) {
...
export function assignPlayerToSlot(player, slotId) {
    const slot = state.plots.slots[slotId];
    if (!slot)
        return { ok: false, message: "Slot not found." };
    if (slot.occupiedByPlayerId)
        return { ok: false, message: "Slot already occupied." };
...
export function teleportPlayerToSlot(player, slotId) {
    const slot = state.plots.slots[slotId];
    if (!slot)
        return { ok: false, message: "Slot not found." };
    const dim = getDimension();
    if (player.dimension.id !== dim.id)
        return { ok: false, message: `Player is in ${player.dimension.id}, slot is in ${dim.id}.` };
...
function structureNameForPlayer(playerId) {
    return `${STORAGE_KEYS.plots.replace(":", "_")}_${playerId}`;
}
function isGeneratorInSlot(generator, slot) {
...
function isLegacySnapshotGenerator(entry) {
    return entry.id !== undefined;
}
function toGeneratorLocationKey(dimensionId, x, y, z) {
    return `${dimensionId}:${Math.floor(x)}:${Math.floor(y)}:${Math.floor(z)}`;
}
function captureSlotGenerators(slot, snapshotOwnerPlayerId) {
...
function removeSlotGenerators(slot) {
    const ids = [];
...
function restoreSlotGenerators(snapshot, slot, ownerPlayerId) {
    if (!snapshot?.generators || snapshot.generators.length === 0)
        return false;
    ...
    for (const entry of snapshot.generators) {
        const normalized = isLegacySnapshotGenerator(entry)
            ? {
                ...
                autoBreakerPurchased: Boolean(entry.autoBreakerPurchased),
                autoBreakerEnabled: Boolean(entry.autoBreakerEnabled),
            }
            : entry;
        const x = slot.min.x + normalized.dx;
        const y = slot.min.y + normalized.dy;
        const z = slot.min.z + normalized.dz;
        if (x < slot.min.x || x > slot.max.x || y < slot.min.y || y > slot.max.y || z < slot.min.z || z > slot.max.z)
            continue;
        ...
        state.generators.placed[id] = {
            ...
            autoBreakerPurchased: Boolean(normalized.autoBreakerPurchased),
            autoBreakerEnabled: Boolean(normalized.autoBreakerEnabled),
        };
        restoredAny = true;
    }
    return restoredAny;
...
function saveSlotSnapshot(slot, playerId, generators) {
    const dim = getDimension();
    const name = structureNameForPlayer(playerId);
    const snapshotGenerators = generators ?? captureSlotGenerators(slot, playerId);
    try {
        dim.runCommand(`structure save ${name} ${slot.min.x} ${slot.min.y} ${slot.min.z} ${slot.max.x} ${slot.max.y} ${slot.max.z} true disk true`);
...
function loadSlotSnapshot(slot, playerId) {
    ...
    try {
        ...
        applyAutoBuildRoof(slot);
        if (restoreSlotGenerators(snap, slot, playerId))
            saveGenerators();
        return true;
    }
    catch {
...
export function processQueuedPlotSnapshots() {
    if (snapshotQueue.length === 0 || !canUsePlotCommandsNow())
        return;
    const retryLimit = 20;
...
function clearSlot(slot) {
    ...
    if (removeSlotGenerators(slot))
        saveGenerators();
}
function saveAndClearSlot(slot, ownerPlayerId) {
    const capturedGenerators = captureSlotGenerators(slot, ownerPlayerId);
    if (!saveSlotSnapshot(slot, ownerPlayerId, capturedGenerators))
        return false;
    clearSlot(slot);
    return true;
}
export function clearAllPlotSlots() {
    const slots = getPlotSlots();
    if (slots.length === 0)
        return { ok: false, message: "No plot slots configured." };
    let cleared = 0;
...
export function clearSlotById(slotId) {
    ...
    slot.occupiedByPlayerId = undefined;
    if (previousOwnerId)
        delete state.plots.playerToSlot[previousOwnerId];
    savePlots();
...
export function assignPlayerToFreeSlot(player) {
    const free = findFreeSlotId();
    if (!free)
        return { ok: false, message: "No available plot slots." };
    return assignPlayerToSlot(player, free);
}
function reconcileOnlinePlotAssignmentsInternal(reason) {
...
export function syncOnlinePlotAssignments() {
    return reconcileOnlinePlotAssignmentsInternal("sync");
}
export function ensureOnlinePlotsAssigned() {
...
export function ensurePlayerPlotAssigned(player) {
...
export function repairPlotSystem() {
...
export function reconcileAllPlotState(reason = "general") {
    if (!state.plots.config.enabled) {
        return {
            ok: true,
            fixed: 0,
    ...
    const ownerRepresentatives = new Map();
    for (const player of world.getAllPlayers()) {
        const playerId = getPlayerId(player);
        const ownerId = getPlotOwnerIdForPlayerId(playerId);
        if (!ownerRepresentatives.has(ownerId))
            ownerRepresentatives.set(ownerId, player);
    }
    let assigned = 0;
    let failed = 0;
    let fixed = (ownership.mappingsFixed ?? 0) +
    ...
    for (const [ownerId, representative] of ownerRepresentatives.entries()) {
        ...
        if (slot && slot.occupiedByPlayerId === ownerId) {
            ...
            if (repairOwnedPlotSlots(ownerId, slot.id))
                fixed += 1;
            continue;
        }
    ...
    if (fixed > 0 || assigned > 0 || failed > 0)
        savePlots();
    cachedPlotSlots = undefined;
    cachedPlotSlotsSource = undefined;
    return {
...
export function clearPlayerPlot(playerId) {
...
export function deployPlayerPlot(player) {
    if (!state.plots.config.enabled)
        return { ok: true, message: "Plots disabled." };
    ...
    const slot = state.plots.slots[slotId];
    if (!slot)
        return { ok: false, message: "Assigned slot not found." };
    const dim = getDimension();
...
export function getPlotOwnerIdForPlayer(player) {
    return getPlotOwnerIdForPlayerId(getPlayerId(player));
}
export function saveAssignedPlayerPlot(player) {
...
export function savePlotAtLocation(location) {
    const slot = getPlotForLocation(location);
    if (!slot || !slot.occupiedByPlayerId)
        return false;
...
export function releasePlayerPlotByName(playerName) {
    const playerId = state.stats.playerIds[playerName];
    if (!playerId)
        return { ok: false, message: "Player ID not found." };
    return releasePlayerPlotById(playerId);
}
export function releasePlayerPlotById(playerId) {
    const slotId = state.plots.playerToSlot[playerId];
    if (!slotId)
        return { ok: false, message: "No assigned plot." };
    const slot = state.plots.slots[slotId];
    if (!slot)
        return { ok: false, message: "Assigned slot missing." };
    if (!saveAndClearSlot(slot, playerId)) {
...
export function showPlotError(player, message) {
    tell(player, `§c[Plots] ${message}`);
}
export function getPlotForLocation(location) {
    for (const slot of getPlotSlots()) {
        if (location.x >= slot.min.x && location.x <= slot.max.x &&
...
export function getPlotTitle(slot) {
    const auto = state.plots.config.autoBuild;
    if (auto.titleMode === "plot")
        return `Plot ${slot.id}`;
    if (!slot.occupiedByPlayerId)
        return `Plot ${slot.id}`;
