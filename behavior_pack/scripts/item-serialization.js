import { EnchantmentTypes, ItemComponentTypes, ItemLockMode, ItemStack } from "@minecraft/server";
function isSerializedVector3(value) {
    if (!value || typeof value !== "object")
        return false;
    const vector = value;
    return typeof vector.x === "number" && typeof vector.y === "number" && typeof vector.z === "number";
}
function toSerializedDynamicValue(value) {
...
function toRuntimeDynamicValue(value) {
    if (typeof value === "boolean")
        return value;
    if (typeof value === "number")
        return value;
    if (typeof value === "string")
        return value;
    if (isSerializedVector3(value))
        return { x: value.x, y: value.y, z: value.z };
    return String(value);
}
function serializeDynamicProperties(stack) {
    try {
...
function safeGetCanDestroy(stack) {
    try {
...
function safeGetCanPlaceOn(stack) {
    try {
...
export function serializeItemStack(stack) {
...
function parseLockMode(raw) {
    if (!raw)
        return undefined;
    if (raw === ItemLockMode.inventory)
        return ItemLockMode.inventory;
    if (raw === ItemLockMode.slot)
        return ItemLockMode.slot;
    if (raw === ItemLockMode.none)
        return ItemLockMode.none;
    return undefined;
}
export function deserializeItemStack(data) {
