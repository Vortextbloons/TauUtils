...
function ensureSidebarDefaults() {
    if (Object.keys(state.sidebars.sidebars).length > 0)
        return;
...
function ensureDefaultSidebarExists() {
    var _a;
    if (state.sidebars.sidebars.main_hud)
        return;
    state.sidebars.sidebars.main_hud = createDefaultSidebar();
    (_a = state.sidebars).defaultSidebarId ?? (_a.defaultSidebarId = "main_hud");
}
...
function getPlayerMoney(player, objectiveId) {
...
function pickSidebarForPlayer(player) {
    if (!state.sidebars.enabled)
        return undefined;
...
function replacePlaceholders(player, sidebar, line) {
...
function buildSidebarLines(player, sidebar) {
    const lines = [];
...
function buildSidebarText(player, sidebar) {
...
function applySidebarForPlayer(player, sidebar) {
...
async function createOrEditSidebar(player, sidebarId) {
    var _a;
    ...
    if (existing.lines.length === 0)
        existing.lines = ["Player: [name]"];
    state.sidebars.sidebars[id] = existing;
    (_a = state.sidebars).defaultSidebarId ?? (_a.defaultSidebarId = id);
    saveSidebars();
    tell(player, `Saved sidebar ${id}.`);
}
async function editSidebarLines(player, sidebar) {
...
async function setSidebarDefault(player) {
    ...
    for (const id of ids)
        picker.button(id, ICONS.sidebar);
    ...
    while (true) {
        ...
        if (response.selection === 2 || response.selection === 3) {
            ...
            if (response.selection === 2)
                await createOrEditSidebar(player, id);
            else
                await editSidebarLines(player, state.sidebars.sidebars[id]);
            continue;
        }
