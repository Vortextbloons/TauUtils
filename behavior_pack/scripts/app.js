...
export { tell, findForm, findShopProfile, canonicalShopId, saveForms, saveShops, saveBinds, saveSidebars, saveRanks, saveChat, getRankById, getPlayerRank, assignRank, removeRank, hasPermission, formatChatMessage, } from "./storage";
...
let startupPhase = 0;
function bootstrap() {
    if (initialized || startupScheduled)
        return;
    startupScheduled = true;
    system.runTimeout(() => {
        if (initialized || startupPhase !== 0)
            return;
        startupPhase = 1;
        loadState();
        ensureDefaults();
