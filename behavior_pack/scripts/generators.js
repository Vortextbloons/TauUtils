import { BlockPermutation, Direction, ItemStack, world } from "@minecraft/server";
import { getPlayerId } from "./storage";
import { getPlayerTeam } from "./teams";
import { getScore, isOperator, saveGenerators, setScore, state } from "./storage";
...
function parsePlotIndex(slotId) {
...
function readGeneratorItemData(itemStack) {
    if (!itemStack)
        return undefined;
    for (const line of itemStack.getLore()) {
        if (!line.startsWith(GENERATOR_MARKER_PREFIX) || !line.endsWith("]"))
            continue;
...
function readGeneratorTier(itemStack) {
...
function readGeneratorAutoBreakerState(itemStack) {
    const data = readGeneratorItemData(itemStack);
    if (data) {
        return {
...
function isAutoBreakerUnlocked(placed, definition) {
...
function normalizePlacedTier(definition, tier) {
    return Math.max(1, Math.min(getMaxTier(definition), Math.floor(tier) || 1));
}
function createPlacedGenerator(def, ownerPlayerId, dimensionId, loc, stack) {
...
function buildGeneratorLore(def, tier, autoBreakerPurchased = false, autoBreakerEnabled = false) {
    const lore = [];
...
function consumePlacedGeneratorItem(player, def) {
    ...
    if (!currentDef || currentDef.id !== def.id)
        return false;
    if (current.amount > 1) {
        current.amount -= 1;
...
function addItemWithoutMergingMetadata(container, stack) {
    for (let slot = 0; slot < container.size; slot++) {
        ...
        if (current.typeId !== stack.typeId || current.amount < 1)
            continue;
        if (String(current.nameTag ?? "") !== String(stack.nameTag ?? ""))
            continue;
        const currentLore = current.getLore();
        const stackLore = stack.getLore();
        if (currentLore.length !== stackLore.length || currentLore.some((line, index) => line !== stackLore[index]))
            continue;
...
function offsetByFace(location, dimensionId, face) {
...
let cachedDefinitionsSource;
function getDefinitions() {
    if (cachedDefinitionsSource === state.generators.definitions && cachedDefinitions)
        return cachedDefinitions;
    cachedDefinitionsSource = state.generators.definitions;
    cachedDefinitions = Object.values(state.generators.definitions);
    return cachedDefinitions;
}
function blockLocationFromVector(location, dimensionId) {
    return {
        dimensionId,
...
function getTier(definition, tier) {
    return definition.tiers.find((entry) => entry.tier === tier);
}
function getGeneratorAtLocation(location) {
    return state.generators.placed[locationKey(location)];
}
function getGeneratorAtLocationKey(key) {
    return state.generators.placed[key];
}
function canPlaceAt(location, player) {
    if (state.generators.config.blockOnPlotOnly && !(player && isOperator(player))) {
        const slot = Object.values(state.plots.slots).slice().sort((a, b) => parsePlotIndex(a.id) - parsePlotIndex(b.id) || a.id.localeCompare(b.id)).find((entry) => location.x >= entry.min.x && location.x <= entry.max.x &&
...
function isSamePlotFootprint(base, output) {
    const basePlot = getPlotForLocation(base);
    const outputPlot = getPlotForLocation(output);
    if (!basePlot && !outputPlot)
        return true;
    if (!basePlot || !outputPlot)
        return false;
    return basePlot.id === outputPlot.id;
}
function clearGeneratorOutput(location, preserveBase = false) {
    try {
        ...
        if (output)
            output.setType("minecraft:air");
    }
    catch {
        // Ignore cleanup failures.
    }
}
function getTopBlock(location) {
    try {
...
function spawnGeneratorOutput(location, definition, amount) {
    try {
        const dim = world.getDimension(location.dimensionId);
...
function collectGeneratorOutput(player, definition, amount) {
    try {
...
export function updateGeneratorDefinition(defId, partial) {
    const def = state.generators.definitions[normalizeId(defId)];
    if (!def)
        return { ok: false, message: "Generator not found." };
    if (partial.name !== undefined)
        def.name = String(partial.name).trim() || def.name;
    if (partial.baseItemId !== undefined)
        def.baseItemId = normalizeItemId(partial.baseItemId);
    if (partial.outputItemId !== undefined)
        def.outputItemId = normalizeItemId(partial.outputItemId);
    if (partial.displayName !== undefined)
        def.displayName = String(partial.displayName).trim() || def.displayName;
    if (partial.placeAnywhere !== undefined)
        def.placeAnywhere = Boolean(partial.placeAnywhere);
    if (partial.lore !== undefined)
        def.lore = partial.lore;
    if (partial.customData !== undefined)
        def.customData = partial.customData;
    if (partial.enchantments !== undefined)
        def.enchantments = partial.enchantments;
    if (partial.durability !== undefined)
        def.durability = partial.durability;
    if (partial.maxDurability !== undefined)
        def.maxDurability = partial.maxDurability;
    if (partial.canPlaceOn !== undefined)
        def.canPlaceOn = partial.canPlaceOn;
    if (partial.canDestroy !== undefined)
        def.canDestroy = partial.canDestroy;
    if (partial.autoBreakerCost !== undefined)
        def.autoBreakerCost = partial.autoBreakerCost;
    cachedDefinitions = undefined;
    cachedDefinitionsSource = undefined;
    saveGenerators();
    return { ok: true, message: `Updated generator ${def.name}.` };
}
export function createGeneratorDefinition(name, baseItemId, outputItemId, rateTicks) {
    const id = normalizeId(name);
    if (!id)
        return { ok: false, message: "Generator name is required." };
    if (state.generators.definitions[id])
        return { ok: false, message: "That generator already exists." };
...
export function addGeneratorTier(defId, rateTicks, upgradeCost) {
    const def = state.generators.definitions[normalizeId(defId)];
    if (!def)
        return { ok: false, message: "Generator not found." };
...
export function updateGeneratorTier(defId, tierNumber, partial) {
    const def = state.generators.definitions[normalizeId(defId)];
    if (!def)
        return { ok: false, message: "Generator not found." };
    ...
    if (partial.rateTicks !== undefined)
        tier.rateTicks = Math.max(1, Math.floor(Number(partial.rateTicks)));
    if (partial.upgradeCost !== undefined)
        tier.upgradeCost = Math.max(0, Math.floor(Number(partial.upgradeCost)));
    saveGenerators();
    return { ok: true, message: `Updated tier ${tier.tier}.` };
}
export function removeGeneratorTier(defId, tierNumber) {
    const def = state.generators.definitions[normalizeId(defId)];
    if (!def)
        return { ok: false, message: "Generator not found." };
    if (def.tiers.length <= 1)
        return { ok: false, message: "A generator must have at least one tier." };
    ...
    for (const placed of Object.values(state.generators.placed)) {
        if (placed.definitionId !== def.id)
            continue;
        if (placed.tier > maxTier)
            placed.tier = maxTier;
    }
    saveGenerators();
    return { ok: true, message: `Removed tier ${tierNumber}.` };
}
export function giveGenerator(player, defId, amount = 1) {
    const def = state.generators.definitions[normalizeId(defId)];
    if (!def)
        return { ok: false, message: "Generator not found." };
    ...
    if (placeComp && def.canPlaceOn && def.canPlaceOn.length > 0)
        placeComp.blocks = def.canPlaceOn;
    const destroyComp = stack.getComponent("minecraft:can_destroy");
    if (destroyComp && def.canDestroy && def.canDestroy.length > 0)
        destroyComp.blocks = def.canDestroy;
    const inventory = player.getComponent("minecraft:inventory")?.container;
    if (!inventory)
        return { ok: false, message: "Inventory unavailable." };
    if (!addItemWithoutMergingMetadata(inventory, stack))
        return { ok: false, message: "Not enough inventory space." };
    return { ok: true, message: `Gave ${amount}x ${def.name}.` };
}
export function getGeneratorInfoLines(defId) {
...
export function getGeneratorTierSummary(defId, tierNumber) {
    const def = getGeneratorDefinition(defId);
    if (!def)
        return undefined;
...
export function updateGeneratorConfig(partial) {
    if (partial.enabled !== undefined)
        state.generators.config.enabled = Boolean(partial.enabled);
    if (partial.defaultPlaceAnywhere !== undefined)
        state.generators.config.defaultPlaceAnywhere = Boolean(partial.defaultPlaceAnywhere);
    if (partial.blockOnPlotOnly !== undefined)
        state.generators.config.blockOnPlotOnly = Boolean(partial.blockOnPlotOnly);
    if (partial.autoBreakersEnabled !== undefined)
        state.generators.config.autoBreakersEnabled = Boolean(partial.autoBreakersEnabled);
    saveGenerators();
    return { ok: true, message: "Generator settings updated." };
}
export function placeGenerator(player, location, dimensionId, itemStack) {
    if (!state.generators.config.enabled)
        return { ok: false, message: "Generators are disabled." };
    const loc = blockLocationFromVector(location, dimensionId);
    const existing = getGeneratorAtLocation(loc);
    if (existing)
        return { ok: false, message: "A generator is already here." };
    const stack = itemStack ?? player.getComponent("minecraft:inventory")?.container?.getItem(player.selectedSlotIndex);
    if (!stack)
        return { ok: false, message: "Hold a generator item to place it." };
    const def = getDefinitionByStack(stack);
    if (!def)
        return { ok: false, message: "That item is not a generator." };
    const outputLoc = { dimensionId: loc.dimensionId, x: loc.x, y: loc.y + 1, z: loc.z };
    if (!isSamePlotFootprint(loc, outputLoc))
        return { ok: false, message: "Generators cannot cross plot boundaries." };
    ...
    if (!def.placeAnywhere && !isAdmin) {
        ...
        if (!plot || plot.occupiedByPlayerId !== plotOwnerId)
            return { ok: false, message: "This generator can only be placed on your plot." };
    }
...
export function pickupGenerator(player, location, dimensionId) {
    const loc = blockLocationFromVector(location, dimensionId);
    const placed = getGeneratorAtLocation(loc);
    if (!placed)
        return { ok: false, message: "No generator there." };
    const playerId = getPlayerId(player);
    const team = getPlayerTeam(player);
    ...
    if (!isOwner && !isOperator(player) && !isTeamMember)
        return { ok: false, message: "Only the owner, team members on team plot, or an operator can pick this up." };
    const def = state.generators.definitions[placed.definitionId];
    if (!def)
        return { ok: false, message: "Generator definition missing." };
    const inventory = player.getComponent("minecraft:inventory")?.container;
    if (!inventory)
        return { ok: false, message: "Inventory unavailable." };
...
export function handleGeneratorUseOnBlock(player, location, dimensionId, itemStack, face) {
    ...
    if (!canPlaceAt(loc, player))
        return { ok: false, message: "You cannot place a generator here." };
    return placeGenerator(player, loc, dimensionId, stack);
}
export function pickupGeneratorIfSneaking(player, location, dimensionId) {
...
export function getPlacedGeneratorAtLocation(location, dimensionId) {
    const loc = blockLocationFromVector(location, dimensionId);
    return getGeneratorAtLocation(loc) ?? getGeneratorAtLocation({ dimensionId, x: loc.x, y: loc.y - 1, z: loc.z });
}
export function getNextUpgradeCost(location, dimensionId) {
    const loc = blockLocationFromVector(location, dimensionId);
    const placed = getGeneratorAtLocation(loc);
    if (!placed)
        return undefined;
...
export function getPlacedGeneratorDefinition(location, dimensionId) {
    const placed = getPlacedGeneratorAtLocation(location, dimensionId);
    if (!placed)
        return undefined;
    return state.generators.definitions[placed.definitionId];
}
export function describeGeneratorStack(itemStack) {
    const def = getDefinitionByStack(itemStack);
    if (!def)
        return undefined;
    return def.name;
}
export function isGeneratorBlock(location, dimensionId) {
    const loc = blockLocationFromVector(location, dimensionId);
    return Boolean(getGeneratorAtLocation(loc) || getGeneratorAtLocation({ dimensionId, x: loc.x, y: loc.y - 1, z: loc.z }));
}
export function upgradeGenerator(player, location, dimensionId) {
    const loc = blockLocationFromVector(location, dimensionId);
    const placed = getGeneratorAtLocation(loc);
    if (!placed)
        return { ok: false, message: "No generator there." };
    ...
    if (!isOwner && !isOperator(player) && !isTeamAccess)
        return { ok: false, message: "Only the owner, team members on team plot, or an operator can upgrade this generator." };
    const def = state.generators.definitions[placed.definitionId];
    if (!def)
        return { ok: false, message: "Generator definition missing." };
    ...
    if (!nextTier)
        return { ok: false, message: "Generator is already at max tier." };
    ...
    if (balance < nextTier.upgradeCost)
        return { ok: false, message: `You need $${nextTier.upgradeCost} to upgrade this generator.` };
...
export function toggleGeneratorAutoBreaker(player, location, dimensionId) {
    const loc = blockLocationFromVector(location, dimensionId);
    const placed = getGeneratorAtLocation(loc);
    if (!placed)
        return { ok: false, message: "No generator there." };
    ...
    const isTeamAccess = canAccessTeamPlotGenerator(player, loc);
    if (!isOwner && !isOperator(player) && !isTeamAccess)
        return { ok: false, message: "Only the owner, team members on team plot, or an operator can change this generator." };
    const def = state.generators.definitions[placed.definitionId];
    if (!def)
        return { ok: false, message: "Generator definition missing." };
    if (!state.generators.config.autoBreakersEnabled)
        return { ok: false, message: "Autobreakers are disabled globally." };
    if (placed.tier < getMaxTier(def))
        return { ok: false, message: "Autobreaker unlocks at max tier." };
    ...
    if (balance === undefined)
        return { ok: false, message: `Missing scoreboard objective "${objective}".` };
    if (balance < cost)
        return { ok: false, message: `You need $${cost} to buy autobreaker.` };
    if (!setScore(player, objective, balance - cost))
        return { ok: false, message: "Failed to deduct autobreaker cost." };
...
export function processGenerators() {
    ...
    for (const placed of Object.values(state.generators.placed)) {
        if (!isGeneratorOwnerActive(placed, onlineIds))
            continue;
        const def = state.generators.definitions[placed.definitionId];
        if (!def)
            continue;
...
export function clearAllGenerators() {
