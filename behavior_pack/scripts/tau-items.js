import { EntityComponentTypes, ItemComponentTypes, ItemStack, system } from "@minecraft/server";
...
const cooldownEndsByKey = new Map();
...
function readRemainingUses(stack) {
    if (!stack)
        return undefined;
    for (const line of stack.getLore()) {
        const trimmed = String(line).trim();
        if (!trimmed.startsWith(TAU_ITEM_USES_PREFIX))
            continue;
...
function writeRemainingUses(stack, remainingUses) {
    ...
    if (marker.trim().length > 0)
        visibleLore.push(marker);
    stack.setLore(visibleLore);
}
function getTauItemFromStack(stack) {
...
function consumeHeldTauItem(player, expected) {
    ...
    if (!held)
        return false;
    if (normalizeItemId(held.typeId) !== normalizeItemId(expected.baseItemId))
        return false;
    if (readTauItemId(held) !== expected.id)
        return false;
...
function damageHeldTauItem(player, expected) {
    const inventory = getInventoryContainer(player);
    if (!inventory)
        return false;
    const held = inventory.getItem(player.selectedSlotIndex);
...
function applyConsumption(player, expected, mode) {
    if (mode === "none")
        return true;
    if (mode === "consume_item")
        return consumeHeldTauItem(player, expected);
    if (mode === "damage_durability")
        return damageHeldTauItem(player, expected);
    return true;
}
function hasRequirements(player, def) {
    ...
    if (def.cost.type === "money") {
        ...
        if (current === undefined)
            return { ok: false, message: `§cMissing objective: ${objective}` };
        if (current < def.cost.amount)
            return { ok: false, message: `§cNeed $${def.cost.amount}` };
        return { ok: true };
    }
    if (def.cost.type === "xp") {
        const level = Number(player.level ?? 0);
        if (level < def.cost.amount)
            return { ok: false, message: `§cNeed ${def.cost.amount} XP levels` };
        return { ok: true };
    }
    if (def.cost.type === "health") {
        ...
        if (current <= def.cost.amount)
            return { ok: false, message: "§cNot enough health" };
        return { ok: true };
    }
    return { ok: true };
}
function payCost(player, def) {
    ...
    if (def.cost.type === "money") {
        const objective = def.cost.objective || state.pay.config.currencyObjective;
        const current = getScore(player, objective);
        if (current === undefined || current < def.cost.amount)
            return false;
...
function renderCommandTemplate(command, player, context) {
...
function runCommandChain(player, commands, context) {
...
function runAction(player, action, context) {
    ...
    if (action.type === "projectile") {
        try {
            ...
            const projectile = player.dimension.spawnEntity(action.entityId, spawnAt);
            const dir = player.getViewDirection();
            const speed = Math.max(0.1, Number(action.speed ?? 1.6));
            if (projectile?.applyImpulse)
                projectile.applyImpulse({ x: dir.x * speed, y: dir.y * speed, z: dir.z * speed });
        }
        catch {
            // ignore
        }
    ...
    if (action.type === "aoe") {
        ...
        for (const target of entities) {
            ...
            if (action.mode === "damage") {
                try {
                    target.applyDamage?.(Math.max(0, Number(action.amount)));
                }
                catch {
                    // ignore
                }
                ...
                try {
                    ...
                    if (typeof target.applyKnockback === "function") {
                        target.applyKnockback(dx / len, dz / len, strength, 0.35);
                    }
                    else {
                        target.applyImpulse?.({ x: (dx / len) * strength, y: 0.25, z: (dz / len) * strength });
                    }
                }
                catch {
                    // ignore
...
function executeTauItem(player, def, context) {
    ...
    if (def.cooldownSeconds > 0)
        setCooldown(player, def.id, def.cooldownSeconds);
    for (const action of def.actions) {
        runAction(player, action, context);
    }
}
export function tryHandleTauItemTrigger(player, trigger, itemStack, context = {}) {
    if (!state.tauItems.config.enabled)
        return { matched: false, cancel: false };
...
export function listTauItemIds() {
    return Object.keys(state.tauItems.items).sort((a, b) => a.localeCompare(b));
}
export function getTauItemDefinition(itemId) {
    return state.tauItems.items[normalizeId(itemId)];
}
export function createTauItemDefinition(id, displayName, baseItemId) {
    const normalized = normalizeId(id);
    if (!normalized)
        return { ok: false, message: "Item id is required." };
    if (state.tauItems.items[normalized])
        return { ok: false, message: "That TauItem already exists." };
...
export function updateTauItemDefinition(itemId, patch) {
    const def = getTauItemDefinition(itemId);
    if (!def)
        return { ok: false, message: "TauItem not found." };
    if (patch.displayName !== undefined)
        def.displayName = String(patch.displayName).trim() || def.displayName;
    if (patch.baseItemId !== undefined)
        def.baseItemId = normalizeItemId(patch.baseItemId);
    if (patch.loreDescription !== undefined)
        def.loreDescription = String(patch.loreDescription);
    if (patch.cooldownSeconds !== undefined)
        def.cooldownSeconds = Math.max(0, Number(patch.cooldownSeconds) || 0);
    if (patch.requiredTag !== undefined)
        def.requiredTag = String(patch.requiredTag).trim() || undefined;
    if (patch.cancelVanilla !== undefined)
        def.cancelVanilla = Boolean(patch.cancelVanilla);
    if (patch.consumption !== undefined)
        def.consumption = patch.consumption;
    if (patch.maxUses !== undefined)
        def.maxUses = Math.max(0, Math.floor(Number(patch.maxUses) || 0)) || undefined;
    if (patch.triggers !== undefined)
        def.triggers = patch.triggers.slice();
    if (patch.actions !== undefined)
        def.actions = patch.actions.slice();
    if (patch.cost !== undefined)
        def.cost = patch.cost;
    saveTauItems();
    return { ok: true, message: `Updated TauItem ${def.id}.` };
}
export function deleteTauItemDefinition(itemId) {
    const id = normalizeId(itemId);
    if (!state.tauItems.items[id])
        return { ok: false, message: "TauItem not found." };
    delete state.tauItems.items[id];
    saveTauItems();
    return { ok: true, message: `Deleted TauItem ${id}.` };
}
function buildTauItemLore(def) {
    const lines = [];
    if (def.loreDescription && def.loreDescription.trim().length > 0)
        lines.push(`§7${def.loreDescription.trim()}`);
    if (def.maxUses !== undefined && def.maxUses > 0)
        lines.push(`§8Uses: §f${def.maxUses}`);
    lines.push(`§8CD: §f${def.cooldownSeconds}s`);
    lines.push(markerLine(def.id));
    return lines;
}
export function giveTauItem(player, itemId, amount = 1) {
    const def = getTauItemDefinition(itemId);
    if (!def)
        return { ok: false, message: `TauItem not found: ${itemId}` };
