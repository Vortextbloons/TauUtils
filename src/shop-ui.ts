...
import { ICONS, type ShopItemDefinition, type ShopItemStackDefinition, type ShopKitDraft, type ShopProfile, type ShopSortMode } from "./tau-models";
import {
  canonicalShopId,
  findShopProfile,
...
function iconForShopItem(item: ShopProfile["items"][number]): string | undefined {
  if (item.displayName && item.displayName.trim().length > 0) return undefined;
  return item.itemId?.trim() || undefined;
}

function shopLabel(item: ShopItemDefinition): string {
  return item.label?.trim() || item.itemId;
}

function shopItemKey(item: ShopItemDefinition): string {
  return item.id?.trim() || `${item.itemId}::${item.label?.trim() || item.displayName?.trim() || ""}::${item.category?.trim() || ""}`;
}

function ensureShopItemId(item: ShopItemDefinition, profileId: string, index: number): string {
...
function getShopItemByKey(profile: ShopProfile, key: string): ShopItemDefinition | undefined {
...
function parseEnchantments(raw: string | undefined): { id: string; level: number }[] {
...
function formatEnchantments(enchantments?: { id: string; level: number }[]): string {
  return (enchantments ?? []).map((entry) => `${entry.id}=${entry.level}`).join(", ");
}

function normalizeItemStackDefinition(def: Partial<ShopItemStackDefinition> & { itemId: string; amount: number }): ShopItemStackDefinition {
  return {
...
function parseItemStackDefinitionLine(raw: string): ShopItemStackDefinition | undefined {
...
function formatItemStackDefinitionLine(def: ShopItemStackDefinition): string {
  return [
    def.itemId,
    String(def.amount),
...
function getItemLore(stack: ItemStack): string[] {
  return stack.getLore().map((line) => String(line).trim()).filter((line) => line.length > 0);
}

function isProtectedCrateKey(stack: ItemStack): boolean {
...
function getItemEnchantments(stack: ItemStack): { id: string; level: number }[] {
...
function getEnchantmentPreviewText(enchantments?: { id: string; level: number }[]): string {
  return (enchantments ?? [])
    .map((entry) => `${entry.id} ${entry.level}`)
    .join(", ");
}

function createStackFromDefinition(def: ShopItemStackDefinition, multiplier = 1): ItemStack {
...
function purchaseStacksAreSingle(def: ShopItemDefinition): boolean {
  const itemId = normalizeItemId(def.itemId);
  return itemId === "enchanted_book" || (def.enchantments?.length ?? 0) > 0;
}

function getMaxStackAmount(itemId: string): number {
  try {
...
function buildChunkedStacks(def: ShopItemStackDefinition, totalAmount: number): ItemStack[] {
...
function itemMatchesDefinition(stack: ItemStack, def: ShopItemStackDefinition): boolean {
  if (isProtectedCrateKey(stack)) return false;
  return normalizeItemId(stack.typeId) === normalizeItemId(def.itemId);
}

function countMatchingItems(player: Player, def: ShopItemStackDefinition): number {
...
function removeMatchingItems(player: Player, def: ShopItemStackDefinition, amount: number): boolean {
  const container = getInventoryContainer(player);
...
function itemRequiresKitMode(item: ShopItemDefinition): boolean {
  return (item.bundle?.length ?? 0) > 0;
}

function singleStackDefinitionFromItem(item: ShopItemDefinition): ShopItemStackDefinition {
...
function buildItemStacksForPurchase(item: ShopItemDefinition, quantity: number): ItemStack[] {
...
function captureHeldItemDefinition(held: ItemStack, amount: number): ShopItemStackDefinition {
...
function snapshotContainer(container: NonNullable<ReturnType<typeof getInventoryContainer>>): InventorySnapshot {
...
function restoreContainer(container: NonNullable<ReturnType<typeof getInventoryContainer>>, snapshot: InventorySnapshot) {
  for (let slot = 0; slot < snapshot.length; slot++) {
    container.setItem(slot, snapshot[slot]?.clone());
  }
}

function getItemInstanceDefinition(item: ShopItemDefinition): ShopItemStackDefinition {
  return normalizeItemStackDefinition({
    itemId: item.itemId,
    amount: 1,
    displayName: item.displayName,
...
function getItemInstanceCount(player: Player, item: ShopItemDefinition): number {
  return countMatchingItems(player, getItemInstanceDefinition(item));
}

function removeItemInstance(player: Player, item: ShopItemDefinition, quantity: number): boolean {
  if (quantity <= 0) return true;
  return removeMatchingItems(player, getItemInstanceDefinition(item), quantity);
}

function promptHeldItemPricing(player: Player, held: ItemStack, preset?: Partial<ShopProfile["items"][number]>) {
...
function categoryList(profile: ShopProfile): string[] {
...
function itemsInCategory(profile: ShopProfile, category: string) {
...
function sortShopItems(items: ShopItemDefinition[], mode: ShopSortMode): ShopItemDefinition[] {
  const sorted = [...items];
  switch (mode) {
    case "name":
...
function normalizeShopProfileItems(profile: ShopProfile): void {
  let changed = false;
