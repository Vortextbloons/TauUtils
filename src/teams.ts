...
function getTeamByPlayerId(playerId: string): TeamDefinition | undefined {
...
function preserveMemberPersonalPlot(team: TeamDefinition, memberId: string): string | undefined {
...
function restoreMemberPersonalPlot(memberId: string, slotId: string | undefined): void {
  if (!slotId) return;
...
function suspendMemberPersonalPlot(team: TeamDefinition, memberId: string): string | undefined {
  if (!team.personalPlotSlotIds) team.personalPlotSlotIds = {};
  const slotId = team.personalPlotSlotIds[memberId] ?? state.plots.playerToSlot[memberId];
  if (!slotId) return undefined;
  team.personalPlotSlotIds[memberId] = slotId;
...
export function reconcileTeamAssignments(): { ok: boolean; fixed: number; removed: number } {
  ...
  if (fixed > 0 || removed > 0) saveTeams();
...
export function createTeam(owner: Player, nameRaw: string, tagRaw?: string): { ok: boolean; message: string } {
  if (!state.teams.enabled) return { ok: false, message: "Teams are disabled." };
  const ownerId = getPlayerId(owner);
  if (state.teams.playerTeamIds[ownerId]) return { ok: false, message: "You are already in a team." };

  const name = String(nameRaw ?? "").trim();
  if (!name) return { ok: false, message: "Team name is required." };
  const id = normalizeTeamId(name);
  if (!id) return { ok: false, message: "Invalid team name." };
  if (state.teams.teams[id]) return { ok: false, message: "That team already exists." };
...
export function inviteToTeam(owner: Player, target: Player): { ok: boolean; message: string } {
  const team = getPlayerTeam(owner);
  if (!team) return { ok: false, message: "You are not in a team." };
  if (team.ownerPlayerId !== getPlayerId(owner)) return { ok: false, message: "Only the owner can invite players." };

  const targetId = getPlayerId(target);
  if (state.teams.playerTeamIds[targetId]) return { ok: false, message: `${target.name} is already in a team.` };
  if (!team.invitedPlayerIds.includes(targetId)) team.invitedPlayerIds.push(targetId);
  saveTeams();
  return { ok: true, message: `Invited ${target.name} to ${team.name}.` };
}

export function revokeTeamInvite(owner: Player, target: Player): { ok: boolean; message: string } {
  const team = getPlayerTeam(owner);
  if (!team) return { ok: false, message: "You are not in a team." };
  if (team.ownerPlayerId !== getPlayerId(owner)) return { ok: false, message: "Only the owner can manage invites." };

  const targetId = getPlayerId(target);
  const before = team.invitedPlayerIds.length;
  team.invitedPlayerIds = team.invitedPlayerIds.filter((id) => id !== targetId);
  if (team.invitedPlayerIds.length === before) return { ok: false, message: `${target.name} did not have a pending invite.` };
  saveTeams();
  return { ok: true, message: `Revoked invite for ${target.name}.` };
}

export function joinTeam(player: Player, teamIdOrName: string): { ok: boolean; message: string } {
  if (!state.teams.enabled) return { ok: false, message: "Teams are disabled." };
  const playerId = getPlayerId(player);
  if (state.teams.playerTeamIds[playerId]) return { ok: false, message: "You are already in a team." };
  ...
  if (team.inviteOnly && !team.invitedPlayerIds.includes(playerId)) return { ok: false, message: "That team is invite-only." };
  if (team.memberPlayerIds.length >= state.teams.maxMembers) return { ok: false, message: "Team is full." };
...
export function leaveTeam(player: Player): { ok: boolean; message: string } {
  const playerId = getPlayerId(player);
  const team = getTeamByPlayerId(playerId);
  if (!team) return { ok: false, message: "You are not in a team." };
  if (team.ownerPlayerId === playerId) return { ok: false, message: "Owner must transfer ownership or disband the team." };
  ...
  delete state.teams.playerTeamIds[playerId];
  if (team.teamPlotEnabled) restoreMemberPersonalPlot(playerId, personalSlotId);
  saveTeams();
  reconcileAllPlotState("team_leave");
  return { ok: true, message: `Left ${team.name}.` };
}

export function kickFromTeam(owner: Player, target: Player): { ok: boolean; message: string } {
  const team = getPlayerTeam(owner);
  if (!team) return { ok: false, message: "You are not in a team." };
  if (team.ownerPlayerId !== getPlayerId(owner)) return { ok: false, message: "Only the owner can kick members." };

  const targetId = getPlayerId(target);
  if (targetId === team.ownerPlayerId) return { ok: false, message: "You cannot kick the owner." };
  if (!team.memberPlayerIds.includes(targetId)) return { ok: false, message: `${target.name} is not in your team.` };
  ...
  if (team.teamPlotEnabled) restoreMemberPersonalPlot(targetId, personalSlotId);
  saveTeams();
  reconcileAllPlotState("team_kick");
  return { ok: true, message: `Kicked ${target.name} from ${team.name}.` };
}

export function disbandTeam(owner: Player): { ok: boolean; message: string } {
  const team = getPlayerTeam(owner);
  if (!team) return { ok: false, message: "You are not in a team." };
  if (team.ownerPlayerId !== getPlayerId(owner)) return { ok: false, message: "Only the owner can disband the team." };
...
export function acceptTeamInvite(player: Player, teamIdOrName: string): { ok: boolean; message: string } {
  ...
  if (!team) return { ok: false, message: "Team not found." };
  const playerId = getPlayerId(player);
  if (!team.invitedPlayerIds.includes(playerId)) return { ok: false, message: "You were not invited to that team." };
  return joinTeam(player, team.id);
}

export function getTeamSummary(team: TeamDefinition): string {
  return `${team.color}[${team.tag}]§r ${team.name} (${team.memberPlayerIds.length}/${state.teams.maxMembers})`;
}

export function setTeamFriendlyFire(owner: Player, enabled: boolean): { ok: boolean; message: string } {
  const team = getPlayerTeam(owner);
  if (!team) return { ok: false, message: "You are not in a team." };
  if (team.ownerPlayerId !== getPlayerId(owner)) return { ok: false, message: "Only the owner can change team settings." };
...
export function setTeamPlotEnabled(owner: Player, enabled: boolean): { ok: boolean; message: string } {
  const team = getPlayerTeam(owner);
  if (!team) return { ok: false, message: "You are not in a team." };
  if (team.ownerPlayerId !== getPlayerId(owner)) return { ok: false, message: "Only the owner can change team settings." };
  ...
  if (enabled) {
    ...
    if (!ownerSlot) return { ok: false, message: "The owner needs an assigned plot first." };
    ...
    if (ownerSlotObj) ownerSlotObj.occupiedByPlayerId = team.ownerPlayerId;
  } else {
    for (const memberId of team.memberPlayerIds) {
