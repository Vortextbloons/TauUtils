...
import { shouldCancelAreaEntityInteract, shouldCancelAreaItemUse } from "../custom-areas";
...
export function canTeleportTo(player: Player, destination: Destination, options: { blockCombat?: boolean } = {}): { ok: boolean; message: string } {
  ...
  if (shouldCancelAreaItemUse(player, location, destination.dimensionId) || shouldCancelAreaEntityInteract(player, location, destination.dimensionId)) {
